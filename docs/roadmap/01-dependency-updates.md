...
- [ ] Every root `pnpm.overrides` entry is an exact version with no range operator (true as of PR 5 and enforced by `pnpm run check-deps`), and all three package manifests match the target policy (pending PR 6 and PR 7).
