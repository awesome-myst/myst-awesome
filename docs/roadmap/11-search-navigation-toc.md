...
- [`NavigationMenu.astro`](https://github.com/awesome-myst/myst-awesome/blob/main/packages/myst-awesome/src/components/NavigationMenu.astro) is a menu component, not a complete project-TOC tree renderer. Its client script binds only the first `.navigation-menu` in the document, and `BasePage` renders the `navigation` slot twice — the desktop sidebar, then the mobile drawer — so the drawer's copy has no search, collapse or keyboard handling. Its filter also hides every section that has no children for any non-empty query, matching or not, so a flat item list (the docs home page's) cannot be searched at all.
- `DocsLayout.astro` renders only its default slot and fills `BasePage`'s `navigation` slot with its own MyST-TOC list, so the `navigation` and `aside` content a page passes is dropped without a warning — `/docs-example` passes a `NavigationMenu` and a `TableOfContents` that never render. Decide whether a page-supplied slot overrides the derived navigation before building `ProjectNavigation` below, since the answer changes what every `DocsLayout` caller sees.
