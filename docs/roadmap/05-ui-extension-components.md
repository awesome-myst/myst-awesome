...
- Deliver complete rendering for `card`, `grid`, `tabSet`, `tabItem`, `dropdown`, `button`/button-styled `link`, and `icon` nodes, plus the `div` and `card` children a grid contains.
...
- A `grid` node has normalized responsive `columns` (one to four breakpoint values, constrained to 1–12). There is no `grid-item` node: the `grid-item` directive emits a `div` carrying `class`, `label`, and `identifier`, and `grid-item-card` emits a `card`, so a grid's children are ordinary nodes rendered in a grid context. [The grid extension source](https://github.com/jupyter-book/mystmd/blob/main/packages/myst-ext-grid/src/index.ts) is the compatibility contract.
...
| Grids | [`myst-ext-grid/src/index.ts`](https://github.com/jupyter-book/mystmd/blob/main/packages/myst-ext-grid/src/index.ts) | `grid.columns`; the `div`/`card` children it emits, with class and target metadata |
...
5. Add `case "grid"` only. Render a semantic `<section>` with `data-columns="1 2 2 3"` (actual normalized values) and CSS variables for the current layout; CSS media queries should select the four upstream breakpoint values. Grid children are `div` and `card` nodes handled by their own branches, so apply `grid-column: span N` from the grid context for a valid span rather than inventing a `grid-item` case for a node type the parser never emits.
...
7. Select exactly one tab per set: the first `tabItem` with `selected: true`, otherwise the first item. `selected` is the MyST AST field, not the emitted attribute — Web Awesome has no `selected` attribute on `wa-tab`. Emit the choice as `active="{id}"` on `wa-tab-group`, and, because these pages are server-rendered, also set the boolean `active` attribute on the matching `wa-tab` and `wa-tab-panel` so the right panel is visible before the group hydrates ([tab group](https://webawesome.com/docs/components/tab-group/), [tab panel](https://webawesome.com/docs/components/tab-panel/)). Preserve `sync` as an escaped `data-myst-sync` hook; synchronization across sets is a small client module, not an SSR guess. `sync` is a grouping key shared by many tabs and must never be used as the `panel`/`name` id.
...
- `myst-zod` already has `tabSet` and `tabItem` schemas in [`src/flow-content/tab-set.ts`](https://github.com/awesome-myst/myst-zod/blob/main/src/flow-content/tab-set.ts), but it does not currently model card, grid, icon, or dropdown-specific data. Its flow-content union is the registration point. [The existing union](https://github.com/awesome-myst/myst-zod/blob/main/src/flow-content/flow-content.ts) must be extended.
