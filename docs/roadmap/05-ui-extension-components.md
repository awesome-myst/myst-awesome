...
6. Add `case "tabSet"` and `case "tabItem"` as a coordinated operation. Generate one collision-free id base per set, render labels as `<wa-tab slot="nav">`, bodies as `<wa-tab-panel>`, and select exactly one tab: the first `selected: true`, otherwise the first item. Preserve `sync` as an escaped `data-myst-sync` hook; synchronization across sets is a small client module, not an SSR guess.
7. Add a `case "admonition"` branch refinement or a dedicated recognized dropdown path that delegates class `dropdown` to `DropdownResolver`. `Dropdown.astro` should render `<wa-details open>` only when the AST says open and use the rendered admonition title as the summary.
...
9. Adjust `case "span"` (after Zod support) so the exact `button` class produces a non-link `<wa-button disabled>` or a presentational button-like span. Do not fabricate a click action for an upstream role with no target.
10. Add `case "icon"` that maps a recognized science icon to `<wa-icon library="scienceicons" name="…">`, maps other upstream `kind` values through an explicit allowlist/configuration table, and falls back to a visually neutral, labelled text marker with a console warning for unsupported libraries.
...
- `packages/myst-awesome/tests/tabs-rendering.spec.ts`: tab roles/panels have matching ids, one selected tab, keyboard activation, and sync data attributes.
