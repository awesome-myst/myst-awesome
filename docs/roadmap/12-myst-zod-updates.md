...
- Correct two frontmatter typings that disagree with the data the schemas produce, found when the dependency roadmap's PR 7 made `astro check` a CI gate. `Person.affiliations` is typed `Affiliation[]`, but parsing keeps an author's affiliation references as ID strings — which is what the theme's `Authors.astro` renders — and `SiteFrontmatter.exports` is declared required, which makes `exports` required on every `ProjectAndPageFrontmatter`. The theme's `frontmatter-demo.astro` carries a commented workaround for each; remove both once the types are fixed.
