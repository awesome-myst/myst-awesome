...
4. Audit [`LaunchButton.astro`](https://github.com/awesome-myst/myst-awesome/blob/main/packages/myst-awesome/src/components/frontmatter/LaunchButton.astro) against the documented Binder/JupyterHub configuration. Keep the existing resolver but add provider-specific URLs, current-page target path, disabled/invalid states, and accessible menu labels. The dependency roadmap's PR 7 removed its reads of `thebe.repository` and `thebe.path`, which MyST's thebe schema does not define, so the repository now comes only from `binder.repo` and the target path needs a real source. Its buttons also build inline `onclick` JavaScript by interpolating frontmatter values into a string; render them as links instead.
