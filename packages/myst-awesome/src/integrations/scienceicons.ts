 ...
 */
function copyScienceIconsLocal(targetBaseDir?: string, baseDir = '') {
  try {
    ...
    if (!fs.existsSync(sourceDir)) {
      ...
      return baseDir ? `${baseDir.startsWith('/') ? baseDir : '/' + baseDir}/scienceicons` : '/scienceicons';
    }
...
export function scienceicons(options: ScienceiconsOptions = {}): AstroIntegration {
  ...
  return {
    ...
    hooks: {
      'astro:config:setup': ({ injectScript, config }) => {
        ...
        try {
          const targetDir = options.targetDir || config.publicDir?.pathname || 'public';
          iconBaseUrl = copyScienceIconsLocal(targetDir, options.baseDir);
